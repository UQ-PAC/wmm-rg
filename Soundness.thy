...
lemma reorder_action:
  assumes "R,G \<turnstile>\<^sub>A P {\<beta>} M" "R,G \<turnstile>\<^sub>A M {\<alpha>} Q" "\<beta> \<hookleftarrow> fwd \<alpha> \<beta>" "inter\<^sub>a R G \<beta> \<alpha>"
  obtains M' where "R,G \<turnstile>\<^sub>A P {fwd \<alpha> \<beta>} M'" "R,G \<turnstile>\<^sub>A M' {\<beta>} Q"
proof -
  ...
  have "R\<^sup>* \<otimes> eval ?\<alpha> \<otimes> R\<^sup>* \<otimes> eval \<beta> \<subseteq> R\<^sup>* \<otimes> eval \<beta> \<otimes> R\<^sup>* \<otimes> eval \<alpha>"
  ...
  hence "P \<subseteq> st R (wp \<beta> (st R (wp \<alpha> Q)))" unfolding st_def wp_def by blast
  hence exec: "P \<subseteq> st R (wp ?\<alpha> (st R (wp \<beta> Q)))" using env by blast
  ...
  moreover have "R,G \<turnstile>\<^sub>A P {?\<alpha>} ?M"
  proof (unfold atomic_rule_def, intro conjI Int_greatest)
...
\<close>
lemma reorder_prog:
  assumes "R,G \<turnstile>\<^sub>t P {c} M" "R,G \<turnstile>\<^sub>A M {\<alpha>} Q" "\<alpha>' < c <\<^sub>p \<alpha>" "inter\<^sub>c R G c \<alpha>"
  obtains M' P' where "P \<subseteq> P'" "R,G \<turnstile>\<^sub>A P' {\<alpha>'} M'" "R,G \<turnstile>\<^sub>t M' {c} Q"
  using assms
...
next
  ...
  have "R,G \<turnstile>\<^sub>t M' {Basic \<beta>} Q" by (rule lrules.basic[OF m''(2)])
...
next
  ...
  obtain M' where m: "R,G \<turnstile>\<^sub>t P {c\<^sub>1} M'" "R,G \<turnstile>\<^sub>t M' {c\<^sub>2} M" using Seq(4) by fast
  ...
  proof (rule Seq(2)[OF _ m(2) Seq(5) \<alpha>(2) i(2)], goal_cases outer)
    case (outer P' N')
    hence c1: "R,G \<turnstile>\<^sub>t P {c\<^sub>1} P'" using m(1) conseq by auto
    show ?case 
...
next
  ...
  obtain m: "R,G \<turnstile>\<^sub>t P {c\<^sub>1} M" "R,G \<turnstile>\<^sub>t P {c\<^sub>2} M" using Choice(4) by blast
  ...
  proof (rule Choice(2)[OF _ m(2) Choice(5) \<alpha>(2) i(2)], goal_cases outer)
    ...
    proof (rule Choice(1)[OF _ m(1) Choice(5) \<alpha>(1) i (1)], goal_cases inner)
      case (inner P'' N'')
      hence "R,G \<turnstile>\<^sub>t (N' \<inter> N'') {c\<^sub>1} Q"
        using outer by (meson Int_lower2 subset_refl lrules.conseq)
      moreover have "R,G \<turnstile>\<^sub>t (N' \<inter> N'') {c\<^sub>2} Q" 
        using inner outer by (meson Int_lower1 subset_refl lrules.conseq)
      ultimately have "R,G \<turnstile>\<^sub>t (N' \<inter> N'') {c\<^sub>1 \<sqinter> c\<^sub>2} Q" by auto
...
next
  case (Loop c)
  then obtain I where i: "P \<subseteq> I" "R,G \<turnstile>\<^sub>t I {c} I" "stable R I" "I \<subseteq> M" by auto
  ...
  proof (rule Loop(1)[OF _ i(2) s(1) \<alpha> d], goal_cases outer)
    ...
    hence "R,G \<turnstile>\<^sub>t (sp \<alpha> R I) {c} (sp \<alpha> R I)" using outer(3) lrules.conseq by simp
    hence "R,G \<turnstile>\<^sub>t (sp \<alpha> R I) {c*} (sp \<alpha> R I)" using s(1) by (meson loop atomic_rule_def)
    hence "R,G \<turnstile>\<^sub>t (sp \<alpha> R I) {c*} Q" using s(2) conseq by blast
...
lemma rewriteI [intro]:
  ...
  assumes "c \<leadsto> c'"
  shows "R,G \<turnstile>\<^sub>t P {c'} Q"
  using assms
...
lemma stepI:
  ...
  shows "\<exists>P' M. P \<subseteq> P' \<and> (R,G \<turnstile>\<^sub>A P' {\<alpha>} M) \<and> (R,G \<turnstile>\<^sub>t M {c'} Q)"
  using assms
...
next
  ...
  obtain M' where m: "R,G \<turnstile>\<^sub>t P {c\<^sub>1} M'" "R,G \<turnstile>\<^sub>t M' {c\<^sub>2} Q" using pre by fast
  then show ?case using pre(2)[OF m(1) pre(4)] m(2) by blast
next
  case (pst c\<^sub>2 \<alpha> c \<alpha>' c\<^sub>2' \<gamma> c\<^sub>1)
  obtain M' where m: "R,G \<turnstile>\<^sub>t P {c\<^sub>1} M'" "R,G \<turnstile>\<^sub>t M' {c\<^sub>2} Q" using pst by fast
  have i: "inter\<^sub>c R G c\<^sub>1 (fwd_prog \<alpha>' c)" "inter\<^sub>c R G c \<alpha>'" using pst by auto
  obtain P' M where m': "M' \<subseteq> P'" "R,G \<turnstile>\<^sub>A P' {\<alpha>} M" "R,G \<turnstile>\<^sub>t M {c\<^sub>2'} Q"
    using pst(2)[OF m(2) i(2)] by blast
  hence m'': "R,G \<turnstile>\<^sub>t P {c\<^sub>1} P'" using m(1) by blast
...
section \<open>Sound Transition Theorem\<close>
